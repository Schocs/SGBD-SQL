--João Pedro Chocron, Priscila Durão, Rafael Gomes Grupo 4
...
--select * from cadastroproduto(registra o codigo do funcionario responsável por cadastrar o código do produto, também registrado na tabela)
...
--select * from pedido(registra o codigo do cliente e data do pedido)
...
--select * from produtopedido(a partir do código do pedido, recebe a quantidade e associa ao produto)
...
from
	produto p inner join cadastroproduto cp on p.codigo = cp.cod_produto 
...
from 
	pedido inner join cliente on pedido.codigo_cliente = cliente.codigo;
...
select 
	...
	produtopedido.quantidade as quantidade_comprada
from
	cliente inner join pedido on cliente.codigo = pedido.codigo_cliente 
	inner join produtopedido on produtopedido.codigo_pedido = pedido.codigo
	inner join produto on produtopedido.codigo_produto=produto.codigo;
	
select 
	distinct cliente.nome as nome_cliente,
	(select count(codigo) from pedido where codigo_cliente=cliente.codigo) as quantidade_pedido
from
	cliente inner join pedido on cliente.codigo=pedido.codigo_cliente
